...
const getWeatherData = async (lat, lng) => {
  ...
  return { city: weatherData.name, weather: weatherData.weather };
};
...
const getDistance = async (origins, destinations) => {
  ...
  const distanceText =
    distanceResponse.data.rows[0].elements[0]?.distance?.text || "Unknown";
  const distanceValue =
    distanceResponse.data.rows[0].elements[0]?.distance?.value || Infinity; // value in meters
